...
namespace IntegrationTesting
{
    public class TestDeleteCredentials
    {
        ...
        {
            Assert.True(authDatabaseUtils.DeleteCredentials("NonExistingUser", PasswordHasher.GetHash("NonExistingUserPass")));
        }

        [Fact]
        public void DeleteExistingUserWithWrongLogin()
        {
            Assert.True(authDatabaseUtils.DeleteCredentials("wrongLogin", PasswordHasher.GetHash(hash_passwords[5])));
        }

        [Fact]
        public void DeleteExistingUserWithWrongPass()
        {
            Assert.True(authDatabaseUtils.DeleteCredentials(logins[6], PasswordHasher.GetHash("wrongPass")));
        }
